...
void View::openFile_clicked() {
  ...
  if (!filename.isEmpty()) {
    try {
      ...
      ui->numVertices->setText(
          "Number of vertices: " +
          QString::number(controller->Instance()->getVertices()->size()) +
          ", ");
      ui->numEdges->setText(
          "Number of edges: " +
          QString::number(controller->Instance()->getIndices()->size() / 3));
...
void View::createGIF_clicked() {
  ...
  QString gif_name = saveGifDialog.getSaveFileName(this, "GIF saving",
                                                   save_filename, suffix_gif);
...
void View::resetGlSettings_clicked() {
  ui->openGLWidget->loadGlSettings();
  setUISettings();
}
...
void View::rotate_valueChanged() {
  ui->openGLWidget->rotateObject(QVector3D(
      ui->rotateX->value(), ui->rotateY->value(), ui->rotateZ->value()));
  ui->openGLWidget->update();
}

void View::move_valueChanged() {
  ui->openGLWidget->moveObject(ui->moveX->value(), ui->moveY->value(),
                               ui->moveZ->value());
  ui->openGLWidget->update();
}
...
void View::setBgColor_clicked() {
  ...
  if (bgColor.isValid()) emit bgColorChanged(bgColor);
}

void View::setPolygonsColor_clicked() {
  QColor pColor = QColorDialog::getColor();
  if (pColor.isValid()) emit polygonsColorChanged(pColor);
}
...
void View::setVerticesColor_clicked() {
  ...
  if (vColor.isValid()) emit verticesColorChanged(vColor);
}
...
void View::initializeUI() {
  ...
  connect(ui->getScreenshot, SIGNAL(clicked()), this,
          SLOT(getScreenshot_clicked()));
  connect(ui->createGIF, SIGNAL(clicked()), this, SLOT(createGIF_clicked()));
  connect(ui->resetParams, SIGNAL(clicked()), this,
          SLOT(resetParams_clicked()));
  connect(ui->resetGlSettings, SIGNAL(clicked()), this,
          SLOT(resetGlSettings_clicked()));
  connect(ui->setBgColor, SIGNAL(clicked()), this, SLOT(setBgColor_clicked()));
  connect(ui->setPolygonColor, SIGNAL(clicked()), this,
          SLOT(setPolygonsColor_clicked()));
  connect(ui->setVerticeColor, SIGNAL(clicked()), this,
          SLOT(setVerticesColor_clicked()));
  // observer
  connect(this, &View::bgColorChanged, ui->openGLWidget,
          &OpenGLWidget::onBgColorChanged);
  connect(this, &View::polygonsColorChanged, ui->openGLWidget,
          &OpenGLWidget::onPolygonsColorChanged);
  connect(this, &View::verticesColorChanged, ui->openGLWidget,
          &OpenGLWidget::onVerticesColorChanged);
  connect(ui->scale, SIGNAL(valueChanged(double)), this,
          SLOT(scale_valueChanged()));
  connect(ui->rotateX, SIGNAL(valueChanged(double)), this,
          SLOT(rotate_valueChanged()));
  connect(ui->rotateY, SIGNAL(valueChanged(double)), this,
          SLOT(rotate_valueChanged()));
  connect(ui->rotateZ, SIGNAL(valueChanged(double)), this,
          SLOT(rotate_valueChanged()));
  connect(ui->moveX, SIGNAL(valueChanged(double)), this,
          SLOT(move_valueChanged()));
  connect(ui->moveY, SIGNAL(valueChanged(double)), this,
          SLOT(move_valueChanged()));
  connect(ui->moveZ, SIGNAL(valueChanged(double)), this,
          SLOT(move_valueChanged()));
  connect(ui->linesWidthSlider, SIGNAL(sliderMoved(int)), this,
          SLOT(linesWidth_sliderMoved(int)));
  connect(ui->dashedPolygonType, SIGNAL(toggled(bool)), this,
          SLOT(polygonsType_toggled(bool)));
  connect(ui->sizeVerticeSlider, SIGNAL(sliderMoved(int)), this,
          SLOT(sizeVertices_sliderMoved(int)));
  connect(ui->noneVertice, SIGNAL(toggled(bool)), this,
          SLOT(verticesType_toggled()));
  connect(ui->circleVertice, SIGNAL(toggled(bool)), this,
          SLOT(verticesType_toggled()));
  connect(ui->squareVertice, SIGNAL(toggled(bool)), this,
          SLOT(verticesType_toggled()));
  connect(ui->orthoProjection, SIGNAL(toggled(bool)), this,
          SLOT(projectionType_toggled(bool)));
  loadSettings();
  setUISettings();
...
void View::loadSettings() {
  ...
  ui->openGLWidget->setBackgroundColor(
      settings->value("bgColor", QColor(230, 230, 230, 255)).value<QColor>());
  ui->openGLWidget->setPolygonsColor(
      settings->value("pColor", QColor(80, 80, 80, 255)).value<QColor>());
  ui->openGLWidget->setVerticesColor(
      settings->value("vColor", QColor(80, 80, 80, 255)).value<QColor>());
  ...
  ui->openGLWidget->setOrthoProjection(
      settings->value("orthoProjToggle", false).toBool());
  ui->openGLWidget->setStippleLines(
      settings->value("stippleLinesToggle", false).toBool());
  ui->openGLWidget->scaleObject(1);
  ui->openGLWidget->saveGlSettings();
}
...
void View::setUISettings() {
  ...
  else if (ui->openGLWidget->getVerticesType() == 2)
...
void View::on_actionOpen_triggered() { openFile_clicked(); }

void View::on_actionSave_as_triggered() { getScreenshot_clicked(); }
