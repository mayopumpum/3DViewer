...
QVector<GLuint> *Model::getIndices() { return &indices; }

void Model::parseVertice(QStringList &line) {
  if (line.size() < 4)
    throw QString("File is damaged!");
  else
    vertices.append(
        QVector3D(line[1].toFloat(), line[2].toFloat(), line[3].toFloat()));
}
...
void Model::centerModel() {
  ...
  QVector<GLfloat> ranges = {vertices[0].x(), vertices[0].x(), vertices[0].y(),
                             vertices[0].y(), vertices[0].z(), vertices[0].z()};
  ...
  float aspect =
      2 / std::max({fabs(ranges[1] - ranges[0]), fabs(ranges[3] - ranges[2]),
...
int Model::getCountVertices() { return vertices.size(); }

int Model::getCountIndices() { return indices.size() / 3; }
